...
class SRTTA():
    def __init__(self, args, model, optimizer, fisher=None, cls_model=None):
        ...
        self.rec_weight = getattr(args, 'rec_weight', 0.0)
    ...
    def __call__(self, train_loader, img_lr, img_gt, img_name, corruption=None):
        ...
        for iteration in range(self.args.iterations):
            self.test_time_adaptation(train_loader, corruptions, img_lr)
    ...
    def test_time_adaptation(self, train_loader, corruption=None, img_lr=None):
        total_loss = 0
        self.model.train()
        ...
        # ############### RECONSTRUCTION LOSS#########################################################################
        if img_lr is not None and getattr(self, 'rec_weight', 0.0) > 0:
            s = (self.args.scale[0]
                if isinstance(self.args.scale, (list, tuple))
                else self.args.scale)
            
            if not self.args.cpu:
                img_lr = img_lr.cuda()

            down = F.interpolate(img_lr,
                                scale_factor=1.0 / s,
                                mode='bicubic',
                                align_corners=False)
            ...
            recon = self.model.sr_forward(down)
            rec_loss = self.compute_loss(recon, img_lr)
